<?php
  ...
  $ref_file = "/data/Calendar/data/notes.json";
  function loadNotes() {
     global $ref_file;
  ...
  function removeNotes( $name ) {
     $d = loadNotes();
     if ( in_array($name, $d) == False) {
        echo("{ \"message\": \"key ".$name." not found in list of notes\"}"); 
        return; // nothing to do
     }
     unset($d[array_search($name,$d)]);
     saveNotes(array_values($d));
  }
